...
import { Activity, Brain, Zap, TrendingUp, Sparkles, Radio, Gauge, Flag, MessageSquare, Cpu, Network, Settings, FileText } from "lucide-react";
...
export default function Dashboard() {
    ...
    const renderSection = () => {
        if (activeSection === "ai-models") return <section className="section-page"><div className="section-page-head"><div><span>AI MODELS</span><h2>Intelligence Stack</h2><p>Every model currently powering PitSense.</p></div><Cpu size={34} /></div><div className="module-grid module-grid-large"><ModuleCard icon={Activity} title="ASR Engine" subtitle="Parakeet TDT 0.6B" color="#1490ff" visual="wave" /><ModuleCard icon={Radio} title="Audio Emotion" subtitle="Wav2Vec2 XLSR" color="#8b4dff" /><ModuleCard icon={Brain} title="Text Emotion" subtitle="DistilRoBERTa" color="#ff8a00" visual="wave" /><ModuleCard icon={Gauge} title="Driver State" subtitle="Intelligence Engine" color="#18b8da" visual="orbit" /><ModuleCard icon={Flag} title="Recommendations" subtitle="Race Engineer AI" color="#3cd05f" visual="wave" /><ModuleCard icon={Sparkles} title="Gemini AI" subtitle="Strategic Advisor" color="#ff2093" visual="wave" /></div></section>;
        if (activeSection === "pipeline") return <section className="section-page"><div className="section-page-head"><div><span>PIPELINE</span><h2>Race Intelligence Flow</h2><p>Radio signal to actionable engineering decision.</p></div><Network size={34} /></div><div className="pipeline-map"><div>RADIO AUDIO</div><b>↓</b><div>PARAKEET ASR</div><b>↓</b><div>AUDIO + TEXT EMOTION</div><b>↓</b><div>DRIVER STATE + STRESS</div><b>↓</b><div>TEMPORAL + TELEMETRY ANALYSIS</div><b>↓</b><div>DECISION + RECOMMENDATION</div></div></section>;
        if (activeSection === "diagnostics") return <section className="section-page"><div className="section-page-head"><div><span>DIAGNOSTICS</span><h2>System Diagnostics</h2><p>Operational checks and local runtime visibility.</p></div><Activity size={34} /></div><div className="diagnostic-grid">{[["Backend","Online","#30d158"],["Parakeet","Local","#1490ff"],["Audio Emotion","Loaded","#8b4dff"],["Text Emotion","Loaded","#ff8a00"],["Session Store","Ready","#18b8da"],["Gemini","Fallback / Optional","#ff2093"]].map(([name,status,color])=><div className="diagnostic-card glass-card" key={name}><span style={{ color }}>{name}</span><strong>{status}</strong><i style={{ background: color }} /></div>)}</div></section>;
        if (activeSection === "api") return <section className="section-page"><div className="section-page-head"><div><span>API REFERENCE</span><h2>PitSense Backend</h2><p>Core endpoints used by the dashboard.</p></div><FileText size={34} /></div><div className="api-list">{[["GET","/health"],["GET","/status"],["GET","/dataset/validate"],["GET","/simulation/samples"],["POST","/upload"],["POST","/chat"],["POST","/session/reset"]].map(([method,path])=><div className="api-row glass-card" key={method+path}><b className={method.toLowerCase()}>{method}</b><code>{path}</code><span>PitSense backend endpoint</span></div>)}</div></section>;
        if (activeSection === "settings") return <section className="section-page"><div className="section-page-head"><div><span>SETTINGS</span><h2>PitSense Control</h2><p>Local runtime preferences.</p></div><Settings size={34} /></div><div className="settings-card glass-card"><div><b>Model execution</b><span>Local Hugging Face models preferred.</span></div><div><b>Gemini synthesis</b><span>Optional remote synthesis / local fallback.</span></div><div><b>Theme</b><span>Dark motorsport cockpit.</span></div></div></section>;
        ...
        return <>
            ...
            {analysis && <TelemetryCard analysis={analysis}/><DecisionCard analysis={analysis}/>}<AISummary analysis={analysis}/><div id="sessions-anchor"><EngineerChat session={activeSession} onUpdateChat={handleUpdateChat}/></div>
